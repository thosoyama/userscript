...
const configs: RollupOptions[] = entryPaths.flatMap((entryPath) => {
  ...
  const mainScriptPath = entryPath
    .replace(/^src\//, 'dist/')
    .replace(/\/(.+)\/main\.ts$/, '/$1.user.js')
  const mainScriptUrl = `file://${rootDir}/${mainScriptPath}`
  const devScriptPath = entryPath
    .replace(/^src\//, 'dist/')
    .replace(/\/(.+)\/main\.ts$/, '/$1.dev.user.js')
  const devfy = (metadata: Metadata): Metadata => {
  ...
  const devConfig: RollupOptions = {
    ...
    output: {
      file: devScriptPath,
      banner: () => `${stringify(devfy(readMetadata(manifestPath)))}\n`,
    },
    plugins: [
      typescript(),
      cleanup({
